        ...
        settings = {
          yazi = {
            ...
            plugin.prepend_previewers =
              let
                namePreviewer = name: run: { inherit name run; };
                mimePreviewer = mime: run: { inherit mime run; };
              in
              [
                (namePreviewer "*.nu" ''piper -- CLICOLOR=1 nu -c "open $1 | nu-highlight"'')
                (namePreviewer "*/" ''piper -- eza -TL=3 --color=always --icons=always --group-directories-first --no-quotes "$1"'')
                (mimePreviewer "image,audio,video}/*" "mediainfo")
                (mimePreviewer "application/x-subrip" "mediainfo")
              ];
          };
