...
let
  ...
  daglib = {
    ...
    isDag = dag: builtins.isAttrs dag && all daglib.isEntry (builtins.attrValues dag);
    ...
    entryAnywhere = daglib.entryBetween [ ] [ ];

    entryAfter = daglib.entryBetween [ ];
    entryBefore = before: daglib.entryBetween before [ ];
      ...
      let
        go =
          ...
          if entries == [ ] then
            ...
            { "${name}" = daglib.dag.entryBetween before after (head entries); }
          else
            {
              "${name}" = daglib.entryAfter after (head entries);
            }
            // go (i + 1) before [ name ] (tail entries);
      in
      go 0;

    entriesAnywhere = tag: daglib.entriesBetween tag [ ] [ ];
    entriesAfter = tag: daglib.entriesBetween tag [ ];
    entriesBefore = tag: before: daglib.entriesBetween tag before [ ];
  };
in
daglib
