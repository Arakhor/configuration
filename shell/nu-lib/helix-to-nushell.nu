...
const SCOPE_MAP = {
  ...
  header: ui.picker.header.column
  separator: ui.virtual.indent-guide
  row_index: ui.linenr
  empty: ui.text.inactive
  leading_trailing_space_bg: ui.virtual.whitespace
...
def get-runtime-dir []: nothing -> path {
  ...
  if not (($local_repo | path exists) and ($"($local_repo)/.git" | path exists)) {
    ...
    if (which git | is-empty) { error make "git is not installed or not in PATH" }
    mkdir $local_repo
...
def resolve-theme-path-by-name [repo: path]: [
  ...
  ls $runtime_dir
  | if ($user_dir | path exists) { merge (ls $user_dir) } else { $in }
  | get name
  | path parse
  | uniq-by stem
...
def parse-theme [repo: path]: [
  ...
  match ($value | describe -d | get type) {
    record => $value
    string => { $value | resolve-theme-path-by-name $repo | open $in }
    path => { open $value }
  }
  | parse-values ($in.palette? | default {})
}
...
def remove-background-from-keys [
  keys: list
]: record -> record {
  let color_config = $in
...
def theme-show-color []: record -> record {
  let prev = $in
  $prev | merge {
    string: {|str|
      if $str =~ '^#[a-fA-F\d]{6}$' {
        ...
        let contrast_white = contrast $str "#ffffff"
        {
          bg: $str
          fg: (
            if ($contrast_black > $contrast_white) { "black" } else { "white" }
          )
        }
      } else {
        $prev.string
